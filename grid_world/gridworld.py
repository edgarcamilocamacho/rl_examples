...
class GridWorld:
    ...
    def step(self, state, action, random=True):
        ...
        if state_ in self.pits:
            reward = -1.0
            done = True
        elif state_ in self.goals:
            reward = 1.0
            done = True
    ...
    def follow_policy(self, init_state=(-1,-1), flag_q=False, gamma=1.0):
        ...
        while True:
            ...
            if done:
                break
            action = self.policy[state]
            state, reward, _, done = self.step(state, action)
